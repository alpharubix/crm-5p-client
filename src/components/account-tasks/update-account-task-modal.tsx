...
export default function UpdateAccountTaskModal({
  ...
  const getLatestNote = (notes: any[]) => {
    ...
    const sorted = [...notes].sort((a: any, b: any) => {
      const dateA =
        a?.Created_Time || a?.Modified_Time || a?.created_at || a?.created_time;
      const dateB =
        b?.Created_Time || b?.Modified_Time || b?.created_at || b?.created_time;
    ...
    const dateStr =
      latestByDate?.Created_Time ||
      latestByDate?.Modified_Time ||
      latestByDate?.created_at;
  ...
  useEffect(() => {
    if (taskData && isOpen) {
      ...
      setTargetCallBackDateTime(taskData.target_call_back_date_time as Date);
  ...
  const canEditOtherFields =
    (isAdminOrManager || isAccountOwner) && !isAssignee;
