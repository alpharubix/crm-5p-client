...
export async function listCibilReports(acc_id: string) {
  const response = await fetch(
    `${ENV.VITE_BACKEND_BASE_URL}/accounts/5pc-crm-list-reports/${acc_id}`,
    { credentials: 'include' },
  )
  if (!response.ok) {
...
export async function getCibilOverview(referenceId: string) {
  const response = await fetch(
    `${ENV.VITE_BACKEND_BASE_URL}/accounts/5pc-r1xcrm-overview/${encodeURIComponent(referenceId)}`,
    { credentials: 'include' },
  )
  if (!response.ok) {
    throw new Error('Failed to fetch date range')
...
export async function getCibilAccountSummary(referenceId: string) {
  const response = await fetch(
    `${ENV.VITE_BACKEND_BASE_URL}/accounts/5pc-r1xcrm-account-summary/${encodeURIComponent(referenceId)}`,
    { credentials: 'include' },
  )
  if (!response.ok) {
    throw new Error('Failed to fetch date range')
...
export async function getCibilPaymentHistory(referenceId: string) {
  const response = await fetch(
    `${ENV.VITE_BACKEND_BASE_URL}/accounts/5pc-r1xcrm-payment-history/${encodeURIComponent(referenceId)}`,
    { credentials: 'include' },
  )
  if (!response.ok) {
    throw new Error('Failed to fetch date range')
...
export async function getCibilAnalysis(referenceId: string) {
  const response = await fetch(
    `${ENV.VITE_BACKEND_BASE_URL}/accounts/5pc-r1xcrm-analysis/${encodeURIComponent(referenceId)}`,
    { credentials: 'include' },
  )
  if (!response.ok) {
    throw new Error('Failed to fetch date range')
